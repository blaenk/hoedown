bitflags! {
    ...
    flags Extension: u32 {
        ...
        #[doc="Enable strikethrough syntax
\
        e.g. `~~strike one~~`"]
        ...
        #[doc="Process highlight syntax
\
        e.g. `==highlight me==`"]
        const HIGHLIGHT             = 1 << 6,

        #[doc="Render quotes differently
\
        example, the html renderer may use the `<q>` tag"]
        const QUOTE                 = 1 << 7,

        #[doc="Process superscript syntax
\
        e.g. `2^3 = 8`"]
        const SUPERSCRIPT           = 1 << 8,

        #[doc="Process math syntax
\
        e.g. `$$x + y = z$$`"]
        const MATH                  = 1 << 9,
        ...
        #[doc="Don't parse emphasis inside of words
\
        e.g. `foo_bar_baz` won't emphasize the 'bar'"]
        const NO_INTRA_EMPHASIS     = 1 << 11,

        #[doc="Process ATX header syntax
\
        e.g. `# Topic`"]
        const SPACE_HEADERS         = 1 << 12,

        #[doc="Process the single dollar math syntax.
\
        e.g. `$x + y = 3$`"]
